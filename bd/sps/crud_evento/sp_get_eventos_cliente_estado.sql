DROP PROCEDURE IF EXISTS sp_get_eventos_cliente_estado;
DELIMITER $$
CREATE PROCEDURE sp_get_eventos_cliente_estado(pClienteID INT, pEstadoID TINYINT, OUT errorMessage VARCHAR(255))
BEGIN
	...
	SELECT ev.id, c.nombreEmpresa AS empresa, ev.nombre AS 'nombre del evento', m.modalidad, te.tipo_evento AS 'tipo de evento',
    ...
    WHERE ev.id_cliente = pClienteID AND ev.id_estado = pEstadoID;
END$$
DELIMITER ;